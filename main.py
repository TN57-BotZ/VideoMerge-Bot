...
async def start_handler(bot: Client, m: Message):
    ...
    await m.reply_text(
        ...
        reply_markup=InlineKeyboardMarkup(
            [
                [InlineKeyboardButton("⚡Leech & Mirror Zone⚡", url="https://t.me/TN57_LEECH_AND_MIRROR_ZONE")],
...
async def callback_handlers(bot: Client, cb: CallbackQuery):
    if "mergeNow" in cb.data:
        ...
        try:
            ...
            await cb.answer("Don't Spam DAW!", show_alert=True)
        ...
        if Config.UPDATES_CHANNEL:
            try:
                ...
                if user.status == "kicked":
                    await cb.message.edit(
                        text="Sorry Sir, You are Banned to use me. Contact my [Support Group](https://t.me/TN57_BotzSupport).",
                        parse_mode="markdown",
                        disable_web_page_preview=True
                    )
                    return
                ...
                await cb.message.edit(
                    text="Something went Wrong. Contact my [Support Group](https://t.me/TN57_BotzSupport).",
                    parse_mode="markdown",
                    disable_web_page_preview=True
                )
                return
        await cb.message.edit(
            text=Config.START_TEXT,
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⚡LEECH & MIRROR⚡", url="https://t.me/TN57_LEECH_AND_MIRROR_ZONE"), InlineKeyboardButton("🎊TN57 BOTZ🎊", url="https://t.me/TN57_Botz")], [InlineKeyboardButton("💢SUPPORT💢", url="https://t.me/TN57_BotzSupport")]]),
            disable_web_page_preview=True
        )
        ...
        try:
            ...
            await cb.answer("Your Queue Empty DAW!", show_alert=True)
        ...
        if (QueueDB.get(cb.from_user.id, None) is not None) or (QueueDB.get(cb.from_user.id) != []):
            ...
            await cb.answer("Sorry DAW, Your Queue is Empty!", show_alert=True)
        ...
        if (QueueDB.get(cb.from_user.id, None) is None) or (QueueDB.get(cb.from_user.id) == []):
            ...
            return
        merged_vid_path = f"{Config.DOWN_PATH}/{str(cb.from_user.id)}/[@TN57_BOTZ]_Merged.{FormtDB.get(cb.from_user.id).lower()}"
        if cb.data.split("_", 1)[-1] == "Yes":
            await cb.message.edit("Okay DAW,\nSend me new file name!")
            try:
